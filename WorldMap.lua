-- Windowed world map and exploration-overlay reveal.
-- Overlay coordinates are derived from ShaguTweaks-extras' Vanilla-compatible map data.

local MAP_OVERLAY_DATA = {
...
local zone, overlays
for zone, overlays in pairs(CUSTOM_MAP_OVERLAY_DATA) do
  MAP_OVERLAY_DATA[zone] = overlays
end
...
local function ApplyWorldMapWindow()
  ...
  if WorldMapFrame.SetClampedToScreen then WorldMapFrame:SetClampedToScreen(true) end
  WorldMapFrame:SetBackdrop(nil)

  local mapWidth = WorldMapButton:GetWidth() or 1002
  local mapHeight = WorldMapButton:GetHeight() or 668
  WorldMapFrame:SetWidth(mapWidth + 16)
  WorldMapFrame:SetHeight(mapHeight + 56)

  local screenWidth = UIParent:GetWidth() or 1024
  local screenHeight = UIParent:GetHeight() or 768
  local targetWidth = screenWidth * .78
  local targetHeight = targetWidth * 9 / 16
  if targetHeight > screenHeight * .78 then targetHeight = screenHeight * .78 end

  local scale = math.min(targetWidth / (mapWidth + 16), targetHeight / (mapHeight + 56))
  scale = scale * .5
  ...
  if not QtUI.worldMapDragHandle then
    ...
    handle:SetFrameLevel(WorldMapFrame:GetFrameLevel() + 50)
  ...
  -- map remains navigable by clicking zones and right-clicking to zoom out.
  ...
  for i = 1, table.getn(regions) do
    ...
    if type(texture) == "string" and string.find(texture, "UI%-WorldMap%-") then
      region:Hide()
    elseif text == WORLD_MAP or text == "World Map" then
      region:Hide()
    end
  end
  ...
  if BlackoutWorld then
    BlackoutWorld:Hide()
    BlackoutWorld:SetScript("OnShow", function() this:Hide() end)
  end
end

local function GetCorrectMapCoordinates(button)
  button = button or WorldMapButton
  ...
  local effectiveScale = uiScale * (WorldMapFrame:GetScale() or 1) * (button:GetScale() or 1)
  local x = cursorX / effectiveScale
  local y = cursorY / effectiveScale
  local centerX, centerY = button:GetCenter()
  local width = button:GetWidth()
  local height = button:GetHeight()
  if not centerX or not centerY or not width or not height or width == 0 or height == 0 then
    return 0, 0
  end

  return (x - (centerX - width / 2)) / width,
    (centerY + height / 2 - y) / height
end

local function CorrectMapHighlight()
  local adjustedX, adjustedY = GetCorrectMapCoordinates(WorldMapButton)
  ...
  if not WorldMapFrame.poiHighlight and WorldMapFrameAreaLabel then
    WorldMapFrameAreaLabel:SetText(name or "")
  end

  if fileName and WorldMapHighlight then
...
local function HookCorrectMapInput()
  ...
  WorldMapButton_OnClick = function(mouseButton, button)
    ...
    if mouseButton == "LeftButton" then
      local x, y = GetCorrectMapCoordinates(button or WorldMapButton)
      ProcessMapClick(x, y)
    else
...
local function HookMapWindowScripts()
  ...
  local originalDown = WorldMapFrame:GetScript("OnMouseDown")
  WorldMapFrame:SetScript("OnMouseDown", function()
    if originalDown then originalDown() end
    if arg1 == "LeftButton" then WorldMapFrame:StartMoving() end
  end)

  local originalUp = WorldMapFrame:GetScript("OnMouseUp")
  WorldMapFrame:SetScript("OnMouseUp", function()
    if originalUp then originalUp() end
    WorldMapFrame:StopMovingOrSizing()
  end)
end

local function ParseOverlay(prefix, entry)
  local _, _, name, width, height, x, y =
    string.find(entry, "^([^:]+):(%d+):(%d+):(%d+):(%d+)")
...
local function DrawRevealedMap()
  ...
  for i = 1, table.getn(zoneData) do
    local textureName, textureWidth, textureHeight, offsetX, offsetY =
      ParseOverlay(prefix, zoneData[i])

    if textureName then
      local horizontal = math.ceil(textureWidth / 256)
      local vertical = math.ceil(textureHeight / 256)
      local row, column

      for row = 1, vertical do
        local pixelHeight = row < vertical and 256 or math.mod(textureHeight, 256)
        if pixelHeight == 0 then pixelHeight = 256 end
        local fileHeight = 16
        while fileHeight < pixelHeight do fileHeight = fileHeight * 2 end

        for column = 1, horizontal do
          local pixelWidth = column < horizontal and 256 or math.mod(textureWidth, 256)
          if pixelWidth == 0 then pixelWidth = 256 end
          local fileWidth = 16
